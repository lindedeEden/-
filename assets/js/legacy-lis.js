...
(function () {
  ...
  function getFilteredRecords() {
    ...
    var rows = getRecords().filter(function (r) {
      ...
      return (r.specimenId || '').toUpperCase().indexOf(q) !== -1
        ...
        || getDisplayNote(r).toUpperCase().indexOf(q) !== -1;
    });
    if (sortDir) {
  ...
  function renderList() {
    ...
    tbody.innerHTML = rows.map(function (r) {
      ...
      var note = getDisplayNote(r);
      var noteCls = note ? 'legacy-lis-note-col has-value' : 'legacy-lis-note-col';
      return '<tr class="' + cls + '" data-id="' + esc(r.specimenId) + '">' +
        ...
        '<td class="' + noteCls + '" title="' + esc(note || '') + '">' + esc(note) + '</td>' +
        '</tr>';
    }).join('');
  ...
  function ensureNoteHeader() {
    var table = document.querySelector('.legacy-lis-list-table thead tr');
    if (!table) return;
    if (!document.getElementById('legacy-lis-col-note')) {
      var th = document.createElement('th');
      th.id = 'legacy-lis-col-note';
      th.textContent = '註記';
  ...
  function ensureShell() {
    ...
    wrap.innerHTML =
      ...
      '<label>檢索 <input type="text" id="legacy-lis-search" placeholder="檢驗單 ID / 病歷號 / 姓名 / 註記"/></label>' +
      ...
      '<th>姓名</th>' +
      '<th id="legacy-lis-col-note">註記</th>' +
      '</tr></thead>' +
